from flask import (
    Blueprint, flash, g, session, request, redirect, url_for, render_template
)
...
def index():
    db = get_db()
    posts = db.execute(
        'SELECT p.id, title, body, created, author_id, username FROM post p JOIN user ON p.author_id = user.id ORDER BY created DESC'
    ).fetchall()
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        elif len(body) <= 0: error = 'Body cannot be empty.'
